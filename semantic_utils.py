...
def get_protected_gaussian_mask(
    ...
    for cam in cameras:
        ...
        with torch.no_grad():
            # Camera-space coords for depth filter.
            # world_view_transform is stored transposed (col-major), so right-multiply.
            xyz_cam = xyz_h @ cam.world_view_transform  # (N, 4)
            depth = xyz_cam[:, 2]                       # Z in camera space
            depth_ok = depth > 0.2

            # Full projection: right-multiply by the transposed full_proj_transform.
            p_hom = xyz_h @ cam.full_proj_transform     # (N, 4)

        # Perspective divide
        p_w = 1.0 / (p_hom[:, 3] + 1e-7)
        p_proj = p_hom[:, :3] * p_w.unsqueeze(1)       # (N, 3), NDC x/y/z

        # NDC [-1,1] -> pixel coords (half-pixel-correct formula)
        u = (((p_proj[:, 0] + 1.0) * W) - 1.0) * 0.5
        v = (((p_proj[:, 1] + 1.0) * H) - 1.0) * 0.5
        u = u.long()
        v = v.long()
        ...
        del xyz_cam, depth, p_hom, p_w, p_proj, u, v, in_bounds
        del mask, u_valid, v_valid, is_fg
        torch.cuda.empty_cache()

    print(f"[semantic_utils] Successfully loaded masks: {loaded_masks_count} / {n_cams} cameras")
